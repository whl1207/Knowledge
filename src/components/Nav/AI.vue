<script setup lang="ts">
    ...
    let func = ref([
        {title:'本地正常对话',link:false,edit:false,online:false,content:""},
        {title:'联网正常对话',link:false,edit:false,online:true,content:""},
        {title:'在文件中，续写文章',link:true,edit:true,online:false,content:"请根据以下文章进行续写。"},
        {title:'在对话中，续写文章',link:true,edit:false,online:false,content:"请根据以下文章进行续写。"},
        {title:'总结文件内容',link:true,edit:false,online:false,content:"请用中文总结下面的段落，并给我一份包含关键信息的列表。"},
        {title:'搜索最新消息',link:false,edit:false,online:true,content:"请根据以下资料进行总结。"},
        {title:'英文翻译',link:false,edit:false,online:false,content:"请将以下段落翻译为英文，要求逻辑通顺，表达自然。"},
        {title:'中文翻译',link:false,edit:false,online:false,content:"请将以下段落翻译为中文，要求逻辑通顺，表达自然。"},
        {title:'指导学习',link:false,edit:false,online:false,content:"我想了解以下主题，请确定并分享从该主题中学到的最重要的部分。"},
        {title:'加强学习',link:false,edit:false,online:false,content:"我目前正在学习以下主题，问我一系列问题来测试我的知识。找出我答案中的知识空白，并给我更好的答案来填补这些空白。"},
        {title:'解释术语',link:false,edit:false,online:false,content:"用任何初学者都能理解的简单易懂的术语解释以下主题。"},
        {title:'重写段落',link:false,edit:false,online:false,content:"重写下面的文字，让初学者容易理解。"},
        {title:'文字游戏',link:false,edit:false,online:false,content:"我想让你扮演一个基于文本的冒险游戏，我在这个冒险游戏中扮演一个角色。游戏中需要有一个最终的任务，完成了最终的任务，即获得游戏的胜利，否则游戏失败。请尽可能具体地描述角色所看到的内容和环境，并在游戏输出的唯一代码块中回复，而不是其它区域。我将输入命令来告诉角色做了什么，而你需要回复角色的行动结果以推动游戏的进行。请从这里开始故事。我的第一个命令是："},
        {title:'剧本杀',link:false,edit:false,online:false,content:"我需要你扮演专业的剧本杀作者，你将会按照以下剧本模版，结合自己的优秀想象力，写出一个包含丰富饱满的剧情、优秀的文笔、逻辑缜密，非常细节的时间线和线索，推理巧妙、立体饱满的人物角色塑造，能让玩家很轻松代入各自角色新颖创意。完整的剧本需要包含以下内容：剧情简介、人物介绍、我扮演的角色和第一幕场景。并且你要回复角色的行动结果以推动游戏的进行。我需要你围绕以下主题进行创作:"},
    ]) as any //功能设置
    ...
    let send_raw = async (prompt:any, keyword:any, QA_history:any, onmessage = alert, addition_args = {}) => {
        ...
        input.value=""
        let result = ''
        let initContent=store.app.files[store.app.fileIndex].content||""
    ...
    const chat=async function(){
        ...
        if(funcIndex.value!=-1){
            str=func.value[funcIndex.value].content
        }
        str=str+input.value
        //使用nlp处理知识库
        ...
        if(iflink.value){
            if(store.app.files.length>0&&store.app.fileIndex>-1){
                if(store.app.files[store.app.fileIndex].type==".md"||store.app.files[store.app.fileIndex].type==".txt"||store.app.files[store.app.fileIndex].type==""){
                    if(!ifNLP.value){
                        //不打开nlp压缩文本
                        str=str+"。参考资料如下："+store.app.files[store.app.fileIndex].content
                    }else{
                        //打开nlp查找最相近的句子
                        let result=await findMostSimilarSentence(input.value, store.app.files[store.app.fileIndex].content)
                        str=str+"。参考资料如下："+result
                        send_raw(str,'',ifhistory.value?history.value:[])
                        return
                    }
                    ...
                    .then(async content => {
                        if(!ifNLP.value){
                            ...
                            //不打开nlp压缩文本
                            str=str+"。参考资料如下："+content.slice(0, strlimit.value);
                        }else{
                            //打开nlp查找最相近的句子
        ...
        if(ifOnline.value){
            str=str+"。参考资料如下："+searchOnline(input.value,onlineNum.value)
        }
        //console.log(str)
        send_raw(str,'',ifhistory.value?history.value:[])
    }
